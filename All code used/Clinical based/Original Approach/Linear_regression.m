%% LINEAR REGRESSION WITH 5-FOLD CROSS-VALIDATION WITH SMOTE

fprintf('\n=== LINEAR REGRESSION WITH 5-FOLD CROSS-VALIDATION WITH SMOTE ===\n');

% Data Preprocessing
Data2_clean = rmmissing(Data2); %very simple command to remove missing entity
...
fold_results = struct();
all_y_true_cv = [];
all_y_pred_scores_cv = [];
all_y_pred_binary_cv = [];

fprintf('\nRunning %d-fold cross-validation with SMOTE...\n', k_folds);
...
for fold = 1:k_folds
    ...
    % === SMOTE APPLICATION: Oversample minority class in training data ===
    fprintf('  Before SMOTE: %d samples (%.1f%% stroke)\n', length(y_train), mean(y_train)*100);
    
    % Apply SMOTE to training data only
    [X_train_smote, y_train_smote] = customSMOTE(X_train, y_train, 5, 0.5);
    
    fprintf('  After SMOTE:  %d samples (%.1f%% stroke)\n', length(y_train_smote), mean(y_train_smote)*100);
    
    % Calculate class weights for this fold
    pos_weight_fold = sum(y_train_smote == 0) / sum(y_train_smote == 1);
    weights_train = ones(size(y_train_smote));
    weights_train(y_train_smote == 1) = pos_weight_fold;
    
    fprintf('  Testing:      %d samples (%.1f%% stroke)\n', length(y_test), mean(y_test)*100);
    
    % Fit linear regression model WITH WEIGHTS
    lm_fold = fitlm(X_train_smote, y_train_smote, 'VarNames', [use_vars, {'stroke'}], 'Weights', weights_train);
    
    % Make predictions on TEST data
    y_pred_test = predict(lm_fold, X_test);
    ...
    [X_roc_fold, Y_roc_fold, T_fold, AUC_fold] = perfcurve(y_test, y_pred_test, 1);
    
    % Calculate F1-score for each threshold
    f1_scores_fold = zeros(length(T_fold), 1);
    for i = 1:length(T_fold)
        y_pred_temp = y_pred_test > T_fold(i);
    ...
    y_pred_binary_fold = y_pred_test > optimal_threshold_fold;
    ...
    brier_fold = mean((y_pred_test - y_test).^2);
    ...
    fold_results(fold).model = lm_fold;
    fold_results(fold).weights = weights_train;
    fold_results(fold).y_pred = y_pred_test;
    ...
    fold_results(fold).residuals = residuals_test;
    
    % Aggregate for overall analysis
    all_y_true_cv = [all_y_true_cv; y_test];
    all_y_pred_scores_cv = [all_y_pred_scores_cv; y_pred_test];
...
% Apply SMOTE to entire dataset for final model training
fprintf('Before SMOTE: %d samples (%.1f%% stroke)\n', length(y), mean(y)*100);
[X_smote_final, y_smote_final] = customSMOTE(X, y, 5, 0.5);
fprintf('After SMOTE:  %d samples (%.1f%% stroke)\n', length(y_smote_final), mean(y_smote_final)*100);

% Calculate class weights for final model
pos_weight_final = sum(y_smote_final == 0) / sum(y_smote_final == 1);
weights_final = ones(size(y_smote_final));
weights_final(y_smote_final == 1) = pos_weight_final;

% Train final model on SMOTE-augmented data
lm_final = fitlm(X_smote_final, y_smote_final, 'VarNames', [use_vars, {'stroke'}], 'Weights', weights_final);

% Make predictions on ORIGINAL data for evaluation
y_pred_final = predict(lm_final, X);
y_pred_final = max(0, min(1, y_pred_final));
...
fprintf('  Stroke cases: %d (%.4f%%)\n', sum(y_smote_final == 1), mean(y_smote_final)*100);
fprintf('  Non-stroke cases: %d (%.4f%%)\n', sum(y_smote_final == 0), (1-mean(y_smote_final))*100);
fprintf('  Class weight for stroke cases: %.2f\n', pos_weight_final);

% Find optimal threshold using F1-Score maximization
[X_roc_final, Y_roc_final, T_final, AUC_final] = perfcurve(y, y_pred_final, 1);

% Calculate F1-score for each threshold
f1_scores = zeros(length(T_final), 1);
for i = 1:length(T_final)
    y_pred_temp = y_pred_final > T_final(i);
...
y_pred_binary_final = y_pred_final > optimal_threshold;
...
brier_final = mean((y_pred_final - y).^2);

% Also calculate AUPRC for final model
[X_pr, Y_pr, ~, AUPRC] = perfcurve(y, y_pred_final, 1, 'XCrit', 'reca', 'YCrit', 'prec');
...
% Main 2x3 plot grid
figure;
sgtitle('Linear Regression for Stroke Prediction (5-Fold CV with SMOTE)', 'FontSize', 16, 'FontWeight', 'bold');

% Plot 1: ROC Curve with optimal threshold (using final model)
subplot(2,3,1);
...
legend(sprintf('Linear Regression with SMOTE (AUC=%.4f)', AUC_final), ...
       'Random Classifier', ...
...
legend('Linear Regression with SMOTE', sprintf('Baseline (%.4f)', baseline_precision), 'Location', 'southwest');

% Plot 3: Probability distributions by class
subplot(2,3,3);
hold on;
if sum(y==0) > 0
    histogram(y_pred_final(y==0), 'BinWidth', 0.02, 'FaceColor', 'blue', 'FaceAlpha', 0.5, 'Normalization', 'pdf');
end
if sum(y==1) > 0
    histogram(y_pred_final(y==1), 'BinWidth', 0.02, 'FaceColor', 'red', 'FaceAlpha', 0.5, 'Normalization', 'pdf');
end
...
% Plot 4: Feature importance for Linear Regression (using final model)
subplot(2,3,4);
base_mse = mean((y_pred_final - y).^2);
perm_importance = zeros(1, min(10, size(X, 2)));

for i = 1:min(10, size(X, 2))
    X_permuted = X;
    X_permuted(:,i) = X_permuted(randperm(size(X_permuted,1)), i);
    perm_pred = predict(lm_final, X_permuted);
    perm_pred = max(0, min(1, perm_pred)); % Clip predictions
    perm_mse = mean((perm_pred - y).^2);
...
title('Linear Regression Feature Importance (SMOTE Model)');
grid on;
hold on;
...
for i = 1:length(thresholds)
    y_pred_temp = y_pred_final >= thresholds(i);
    TP = sum(y_pred_temp & y);
    FP = sum(y_pred_temp & ~y);
    FN = sum(~y_pred_temp & y);
    precision_temp = TP / (TP + FP + eps);
...
for i = 1:length(bin_centers)
    in_bin = y_pred_final >= bin_edges(i) & y_pred_final < bin_edges(i+1);
    if sum(in_bin) > 0
        mean_predicted(i) = mean(y_pred_final(in_bin));
...
legend('Linear Regression with SMOTE', 'Perfect Calibration', 'Location', 'northwest');
grid on;
...
y_pred_jittered = y_pred_final + 0.01 * randn(size(y_pred_final)); % Very small jitter to predictions
...
plot([-0.2, 1.2], [optimal_threshold, optimal_threshold], 'r--', 'LineWidth', 2, 'HandleVisibility', 'off');
% Add vertical decision line
plot([0.5, 0.5], [-0.1, 1.1], 'k--', 'LineWidth', 1, 'HandleVisibility', 'off');
...
subplot(1,2,2);
boxplot(y_pred_final, y_numeric, 'Labels', {'No Stroke', 'Stroke'});
xlabel('Actual Class');
...
figure;
sgtitle('5-Fold Cross-Validation Analysis with SMOTE', 'FontSize', 16, 'FontWeight', 'bold');
...
plot(1:k_folds, fold_metrics_matrix, 'o-', 'LineWidth', 2, 'MarkerSize', 8);
xlabel('Fold Number');
ylabel('Score');
title('Performance Metrics Across Folds');
legend(metrics, 'Location', 'best');
grid on;
xlim([0.5, k_folds+0.5]);

% Plot optimal thresholds across folds
subplot(2,3,2);
bar(1:k_folds, [fold_results.optimal_threshold]);
hold on;
plot(xlim(), [optimal_threshold optimal_threshold], 'r--', 'LineWidth', 2);
xlabel('Fold Number');
...
% Plot feature stability across folds (coefficient variation)
subplot(2,3,4);
% Collect coefficients from all folds
coeffs_all = zeros(k_folds, length(use_vars));
for fold = 1:k_folds
    coeffs_all(fold, :) = fold_results(fold).model.Coefficients.Estimate(2:end)'; % Skip intercept
end

% Calculate coefficient of variation for each feature
coeff_cv = std(coeffs_all, 0, 1) ./ abs(mean(coeffs_all, 1));
...
% Plot residuals distribution from cross-validation
subplot(2,3,5);
all_residuals_cv = [];
for fold = 1:k_folds
    all_residuals_cv = [all_residuals_cv; fold_results(fold).residuals];
end
histogram(all_residuals_cv, 30, 'FaceColor', 'blue', 'FaceAlpha', 0.7);
xlabel('Residuals');
ylabel('Frequency');
title('Residuals Distribution (Cross-Validation with SMOTE)');
grid on;

% Add normal distribution fit
hold on;
x = linspace(min(all_residuals_cv), max(all_residuals_cv), 100);
mu = mean(all_residuals_cv);
sigma = std(all_residuals_cv);
y_norm = length(all_residuals_cv) * (max(all_residuals_cv)-min(all_residuals_cv))/30 * ...
         normpdf(x, mu, sigma);
plot(x, y_norm, 'r-', 'LineWidth', 2);
legend('Residuals', sprintf('Normal Fit (μ=%.3f, σ=%.3f)', mu, sigma), 'Location', 'northwest');
...
fprintf('(With SMOTE oversampling and F1-optimized threshold)\n\n');
...
fprintf('\n=== SMOTE IMPACT ANALYSIS ===\n');
fprintf('Expected improvements with SMOTE:\n');
fprintf('1. Increased Recall: Model learns better from minority class\n');
fprintf('2. Better F1-Score: More balanced precision-recall tradeoff\n');
fprintf('3. More Robust Model: Less biased toward majority class\n');
fprintf('4. Better Generalization: Learns true decision boundaries\n');

fprintf('\nSMOTE Parameters Used:\n');
fprintf('- k-nearest neighbors: 5\n');
fprintf('- Oversampling ratio: 0.5 (target 1:2 stroke:non-stroke ratio)\n');
fprintf('- Applied only to training data in cross-validation\n');
fprintf('- Final model trained on SMOTE-augmented full dataset\n');

fprintf('\n=== LINEAR REGRESSION WITH 5-FOLD CV WITH SMOTE ANALYSIS COMPLETE ===\n');
