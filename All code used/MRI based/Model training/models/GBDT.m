%% Aneurysm MRI Dataset Analysis - Gradient Boosting with SMOTE and 5-Fold CV
...
fprintf('\n========== NORMAL vs ANEURYSM CLASSIFICATION (GRADIENT BOOSTING) ==========\n');
...
fprintf('\n========== 5-FOLD CROSS-VALIDATION WITH GRADIENT BOOSTING ==========\n');
...
for fold = 1:5
    ...
    % Train Gradient Boosting for this fold - FIXED: Use correct binary methods
    % Options: 'AdaBoostM1', 'LogitBoost', 'GentleBoost', 'RobustBoost'
    methods = {'AdaBoostM1', 'LogitBoost', 'GentleBoost'};
    learningRates = [0.05, 0.1, 0.2];
    numLearners = [50, 100, 150];
    minLeafSizes = [5, 10, 20];
    ...
    for m = 1:length(methods)
        for lr = 1:length(learningRates)
            for nl = 1:length(numLearners)
                for ml = 1:length(minLeafSizes)
                    try
                        % Create template tree for boosting
                        t = templateTree('MinLeafSize', minLeafSizes(ml), 'NumVariablesToSample', 'all');
                        
                        tempModel = fitcensemble(data_train, 'Target', ...
                            'Method', methods{m}, ...
                            'NumLearningCycles', numLearners(nl), ...
                            'Learners', t, ...
                        ...
                        if temp_auc > bestAUC_fold
                            ...
                            bestParams_fold = sprintf('%s, LR=%.2f, Learners=%d, MinLeaf=%d', ...
                                methods{m}, learningRates(lr), numLearners(nl), minLeafSizes(ml));
                        end
                    catch ME
    ...
    if isempty(bestModel_fold)
        fprintf('  Using default Gradient Boosting (LogitBoost)\n');
        t = templateTree('MinLeafSize', 10);
        foldModel = fitcensemble(data_train, 'Target', ...
            'Method', 'LogitBoost', ...
...
% Find best hyperparameters for final model
methods = {'AdaBoostM1', 'LogitBoost', 'GentleBoost'};
learningRates = [0.05, 0.1, 0.2];
...
bestAUC_final = 0;
best_final_model = [];
best_method = 'LogitBoost';
best_lr = 0.1;
best_nl = 100;
best_ml = 10;

fprintf('Tuning hyperparameters for final model...\n');
        ...
        for nl = 1:length(numLearners)
            for ml = 1:length(minLeafSizes)
                try
                    ...
                    tempModel = fitcensemble(data_final, 'Target', ...
                        'Method', methods{m}, ...
                        'NumLearningCycles', numLearners(nl), ...
                        'Learners', t, ...
                        'LearnRate', learningRates(lr), ...
                    ...
                    if temp_auc > bestAUC_final
                        ...
                        best_method = methods{m};
                        best_lr = learningRates(lr);
                        best_nl = numLearners(nl);
                        best_ml = minLeafSizes(ml);
                        fprintf('  New best: %s, LR=%.2f, Learners=%d, MinLeaf=%d (AUC=%.4f)\n', ...
                            best_method, best_lr, best_nl, best_ml, temp_auc);
                    end
                catch ME
                    continue;
                end
...
fprintf('\nSelected: %s, LR=%.2f, Learners=%d, MinLeaf=%d (AUC=%.4f)\n', ...
    best_method, best_lr, best_nl, best_ml, bestAUC_final);
...
%% 13. Plot Results
figure('Name', 'Gradient Boosting Results', 'Position', [100, 100, 1600, 900]);
...
subplot(2, 3, 2);
bar(1:5, cvAUCs, 'FaceColor', [0.2, 0.6, 0.8]);
hold on;
...
% Feature Importance (using correlation as fallback)
subplot(2, 3, 3);
% Use correlation-based importance (always works)
correlations = abs(corr(X_selected, y));
[imp_sorted, idx] = sort(correlations, 'descend');
...
fprintf('Model Type: Gradient Boosting\n');
fprintf('Method: %s\n', best_method);
...
fprintf('\n Analysis complete! FINAL GRADIENT BOOSTING AUC = %.4f\n', final_auc);