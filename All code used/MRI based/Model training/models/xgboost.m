%% Aneurysm MRI Dataset Analysis - XGBoost-style Gradient Boosting with SMOTE and 5-Fold CV (OPTIMIZED)
...
fprintf('\n========== NORMAL vs ANEURYSM CLASSIFICATION (XGBOOST-STYLE) ==========\n');
...
fprintf('\n========== 5-FOLD CROSS-VALIDATION WITH XGBOOST-STYLE (OPTIMIZED) ==========\n');
...
% Pre-defined reasonable hyperparameters (no tuning during CV)
% Based on typical XGBoost values for medical datasets
learningRate = 0.1;      % Standard learning rate
maxDepth = 6;            % Convert to MaxNumSplits = 2^6 - 1 = 63
numTrees = 150;          % Good number of trees
minLeafSize = 10;        % Prevent overfitting

fprintf('Using fixed hyperparameters for speed:\n');
fprintf('  Learning Rate: %.2f\n', learningRate);
fprintf('  Max Depth: %d\n', maxDepth);
fprintf('  Number of Trees: %d\n', numTrees);
fprintf('  Min Leaf Size: %d\n', minLeafSize);

%% 10. Perform 5-Fold Cross-Validation (No tuning for speed)
for fold = 1:5
    fprintf('\n--- Fold %d of 5 ---\n', fold);
    ...
    % Convert maxDepth to MaxNumSplits
    maxSplits = 2^maxDepth - 1;
    
    % Create template tree with XGBoost-like parameters
...
%% 12. Train Final Model on ALL Data (with minimal tuning)
fprintf('\n========== TRAINING FINAL XGBOOST-STYLE MODEL ==========\n');
...
% Quick tuning with just 3 options for learning rate
learningRate_options = [0.05, 0.1, 0.2];
bestAUC_final = 0;
best_final_model = [];
best_lr = 0.1;

fprintf('Quick tuning learning rates...\n');

for lr = 1:length(learningRate_options)
    try
        maxSplits = 2^maxDepth - 1;
        ...
        tempModel = fitcensemble(data_final, 'Target', ...
            ...
            'NumLearningCycles', numTrees, ...
            'Learners', t, ...
            'LearnRate', learningRate_options(lr), ...
            'ClassNames', [0; 1]);
        ...
        fprintf('  LR=%.2f: AUC=%.4f\n', learningRate_options(lr), temp_auc);
        
        if temp_auc > bestAUC_final
            bestAUC_final = temp_auc;
            best_final_model = tempModel;
            best_lr = learningRate_options(lr);
        end
    catch ME
        fprintf('  LR=%.2f failed\n', learningRate_options(lr));
    end
end

finalModel = best_final_model;
fprintf('\nSelected LR=%.2f (AUC=%.4f)\n', best_lr, bestAUC_final);
...
fprintf('\n========== FINAL XGBOOST-STYLE MODEL PERFORMANCE ==========\n');
...
%% 13. Plot Results
figure('Name', 'XGBoost-style Results', 'Position', [100, 100, 1600, 900]);
...
subplot(2, 3, 2);
bar(1:5, cvAUCs, 'FaceColor', [0.9, 0.6, 0.3]);
hold on;
...
%% 14. Final Summary
fprintf('\n========== XGBOOST-STYLE MODEL SUMMARY ==========\n');
fprintf('Model Type: Gradient Boosting (LogitBoost)\n');
fprintf('Learning Rate: %.2f\n', best_lr);
fprintf('Max Depth: %d\n', maxDepth);
fprintf('Number of Trees: %d\n', numTrees);
fprintf('Min Leaf Size: %d\n', minLeafSize);
...
fprintf('\n Analysis complete! FINAL XGBOOST-STYLE MODEL AUC = %.4f\n', final_auc);