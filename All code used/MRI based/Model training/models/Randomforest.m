%% Aneurysm MRI Dataset Analysis - Random Forest with SMOTE and 5-Fold CV
...
fprintf('\n========== NORMAL vs ANEURYSM CLASSIFICATION (RANDOM FOREST) ==========\n');
...
for fold = 1:5
    ...
    t = templateTree('MinLeafSize', 1, 'NumVariablesToSample', 'all');
    foldModel = fitcensemble(data_train, 'Target', ...
        'Method', 'Bag', ...
        'NumLearningCycles', 200, ... % Slightly fewer trees for CV speed
        'Learners', t, ...
        'ClassNames', [0; 1]);
...
%% 12. Train Final Model on ALL Data (for comparison)
...
t = templateTree('MinLeafSize', 1, 'NumVariablesToSample', 'all');
finalModel = fitcensemble(data_final, 'Target', ...
    'Method', 'Bag', ...
...
%% 13. Plot Results - UPDATED with FINAL MODEL ROC as PRIMARY
figure('Name', 'Random Forest CV Results', 'Position', [100, 100, 1600, 900]);

% ROC Curve - FINAL MODEL (PRIMARY)
subplot(2, 3, 1);
[X_roc_final, Y_roc_final, ~, ~] = perfcurve(y, yScore_final, 1);
plot(X_roc_final, Y_roc_final, 'b-', 'LineWidth', 4);
hold on;
% Add CV pooled ROC for comparison (dashed)
...
subplot(2, 3, 2);
bar(1:5, cvAUCs, 'FaceColor', [0.2, 0.6, 0.2]);
hold on;
...
[corr_sorted, idx] = sort(correlations, 'descend');
...
%% 14. Final Summary with CV Comparison
fprintf('\n========== RANDOM FOREST MODEL SUMMARY ==========\n');
fprintf('Model Type: Random Forest (fitcensemble)\n');
fprintf('Number of trees: 300 (CV: 200 per fold)\n');
fprintf('Min leaf size: 1\n');
...
fprintf('\n Analysis complete! FINAL MODEL AUC = %.4f (vs CV average %.4f)\n', ...
    final_auc, mean(cvAUCs));