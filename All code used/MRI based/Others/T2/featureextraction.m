...
% Adapted for aneurysm detection with 12-region analysis
...
% ANEURYSM DETECTION FOR ALL SLICES (USING REGION-BASED CODE)
fprintf('\n%s\n', repmat('-', 1, 80));
fprintf('ANEURYSM DETECTION FOR ALL SLICES\n');
fprintf('%s\n', repmat('-', 1, 80));
...
for slice_idx = 1:n_slices
    fprintf('\n--- Processing Slice %d/%d for Aneurysm Detection ---\n', slice_idx, n_slices);
    ...
    brain_mask = skull_strip_improved(original);
    interior_mask = create_brain_interior_mask(brain_mask, 10);
    [~, region_masks, ~] = split_into_focused_regions(interior_mask, 12);
    
    % Extract vessel properties and detect aneurysms using region-based code
    [aneurysm_mask, aneurysm_regions] = detect_aneurysms_in_slice(original, brain_mask, interior_mask, region_masks);
...
save('aneurysm_detection_results.mat', 'aneurysm_regions_by_slice', 'aneurysm_masks_by_slice');
fprintf('\nAneurysm detection results saved\n');
...
for slice_idx = 1:n_slices
    ...
    % Wavelet denoising
    brain_denoised = wavelet_denoise(original);
    
    % Skull stripping
    brain_mask = skull_strip_improved(brain_denoised);
    ...
    % VESSEL EXTRACTION (USING REGION-BASED APPROACH)
    ...
    % Extract features
    region_features = extractFeaturesWithAneurysm(vessel_mask, interior_mask, brain_enhanced, vesselness_map, ...
...
fprintf('\nAll regions have been labeled based on aneurysm detection\n');

% ANEURYSM DETECTION FUNCTION (WRAPPER FOR REGION-BASED CODE)
function [aneurysm_mask, aneurysm_regions] = detect_aneurysms_in_slice(img, brain_mask, interior_mask, region_masks)
    ...
    vessel_props = extract_vessel_properties_simple(img, interior_mask);
    ...
    for r = 1:length(region_masks)
        ...
        [region_aneurysm, has_aneurysm] = find_region_aneurysms(region_vessels, vessel_props.response, region_mask);
...
% SIMPLE VESSEL PROPERTIES EXTRACTION (FOR ANEURYSM DETECTION)
function vessel_props = extract_vessel_properties_simple(img, interior_mask)
...
% FIND REGION-SPECIFIC ANEURYSMS
function [aneurysm_mask, has_aneurysm] = find_region_aneurysms(vessel_mask, vessel_response, region_mask)
...
% FEATURE EXTRACTION FUNCTION (ADAPTED FOR ANEURYSM) - FIXED VERSION
function region_features = extractFeaturesWithAneurysm(vessel_mask, interior_mask, brain_enhanced, vesselness_map, ...
    ...
    for r = 1:grid_rows
        for c = 1:grid_cols
            ...
            region_feat = extractCoreFeatures(region_vessels, region_mask, brain_enhanced, vesselness_map);
            ...
            % Append to results - FIX: Check if columns match
...
% FUNCTION: Core Feature Extraction (ENHANCED FOR ANEURYSM) - FIXED
function features = extractCoreFeatures(vessel_mask, brain_mask, brain_enhanced, vesselness_map)
    features = struct();
    ...
    if num_vessels == 0
        features = setDefaultNaNFeatures(features);
        return;
    end
    ...
    % 5. VESSELNESS CHARACTERISTICS (from Frangi filter)
...
% HELPER: Set Default NaN Features - FIXED to return features struct
function features = setDefaultNaNFeatures(features)
...
function brain_mask = skull_strip_improved(mri_image)
...
function blended_vesselness = analyze_regions_improved(img, interior_mask, num_regions)
    ...
    for r = 1:num_valid_regions
        ...
        region_frangi = frangi_vesselness(region_img);
...
function vesselness = frangi_vesselness(img)
