% T2 BRAIN ANEURYSM DETECTION - WITH REGION SPLITTING (12 REGIONS)
% Based on vessel extraction code with region splitting and blending
...
if isempty(files)
    error('No image files found. Place a T2 image in current folder.');
end
...
function brain_mask = skull_strip_t2(mri_image)
...
function denoised_img = wavelet_denoise_t2(img)
...
fprintf('\n%s\n', repmat('-', 1, 70));
fprintf('T2 ANEURYSM DETECTION - WITH REGION SPLITTING (12 REGIONS)\n');
fprintf('%s\n', repmat('-', 1, 70));

% Step 1: Skull stripping
fprintf('\nStep 1: Removing skull...\n');
brain_mask = skull_strip_t2(original);
brain_only = original;
...
brain_denoised = wavelet_denoise_t2(brain_only);
...
figure('Name', 'T2 Aneurysm Detection - Region-Based Analysis', ...
...
title('A. ORIGINAL T2 IMAGE', 'FontSize', 14, 'FontWeight', 'bold');
axis image off;
...
sgtitle('T2 Brain Aneurysm Detection - Region-Based Analysis (12 Regions)', ...
